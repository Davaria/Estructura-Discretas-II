...
void prim(const Graph &graph)
{
  ...
  */
  ...
  // Elija la parte superior inicial
  const int startVertex = 0;
  visitedVertices.insert(startVertex);

  const int maxEdgeWeight = 1000000;

  // variables donde se almacenan las tapas del borde más adecuado
  int firstVertexOfEdge{0}, secondVertexOfEdge{0}, weightVertexOfEdge{0};

  // para encontrar un peso mínimo
  int minEdgeWeight{0};

  ofstream prim;
  prim.open("prim.dot");
  prim << "graph A {" << endl;
  while (visitedVertices.size() < e)
  {
    // establecer el peso más alto para encontrar el peso mínimo
    minEdgeWeight = maxEdgeWeight;

    // ejecutar todos los nodos visitados
    for (const auto &i : visitedVertices)
    {
      // todos adyacentes a los nodos visitados, excluyendo los ya visitados
      // Si se encuentra una nodo menor
      for (int j = 0; j < e; j++)
      {

        if ((result[j].weight) && (visitedVertices.find(j) == visitedVertices.end()) &&
            (minEdgeWeight > (result[j].weight)))
        {
          minEdgeWeight = result[j].weight;
          firstVertexOfEdge = i;
    ...
    cout << firstVertexOfEdge << " -- " << secondVertexOfEdge << " == " << minEdgeWeight << endl;
    prim << "\t" << firstVertexOfEdge << " --" << secondVertexOfEdge << " [label = \"" << minEdgeWeight << "\" ];" << endl;

    // añadir a los nodos visitados la parte superior, adyacente del vector
